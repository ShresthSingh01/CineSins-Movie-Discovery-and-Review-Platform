        ...
        return new Promise(async (resolve) => {
            ...
            if (token && active) {
                ...
                if (gate) {
                    ...
                    // We have the original "Who is watching?" HTML from index.html here.
                    ...
                    if (profilesList) {
                        try {
                            ...
                            profilesList.innerHTML = profiles.map(p => `
                                <div class="profile-card" onclick="window.selectProfile('${p.id}')" style="cursor: pointer; padding: 15px; border-radius: 12px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); transition: all 0.3s;">
                                    <div style="width: 60px; height: 60px; border-radius: 50%; background: #db2777; margin: 0 auto 10px; display: flex; align-items: center; justify-content: center; font-size: 24px; font-weight: bold; color: white;">
                                        ${p.name.charAt(0).toUpperCase()}
                                    </div>
                                    <h3 style="color: #fff; font-size: 1rem; margin: 0;">${p.name}</h3>
                                </div>
                            `).join('');
                            ...
                            console.error("Error rendering profiles:", e);
                        }
                    }

                    if (createBtn && newProfileInput) {
                        createBtn.onclick = async () => {
                            const name = newProfileInput.value.trim();
                            if (!name) return alert("Profile name cannot be empty");
                            createBtn.textContent = "Creating...";
                            try {
                                const newP = await this.createProfile(name);
                                if (newP) {
                                    await this.switchProfile(newP.id);
                                }
                            } catch (e) {
                                alert("Failed to create profile: " + e.message);
                                createBtn.textContent = "Create & Select";
                            }
                        };
                    }
                }
                ...
                if (gate) {
                    gate.style.display = 'flex';
                    // Render explicit login screen
                    gate.innerHTML = `
                        <div class="auth-container" style="background: rgba(0,0,0,0.8); padding: 30px; border-radius: 12px; border: 1px solid #333; width: 300px; text-align: center; margin: auto; box-shadow: 0 30px 60px rgba(0,0,0,0.8);">
                            <h2 style="color: #fff; font-family: 'Playfair Display', serif; margin-bottom: 20px;">Login</h2>
                            <p style="color: var(--cin-muted); font-size: 0.8rem; margin-bottom: 20px;">If the account doesn't exist, we'll create it.</p>
                            <input type="email" id="auth-email" placeholder="Email" style="width: 100%; padding: 12px; margin: 10px 0; background: #111; color: #fff; border: 1px solid #444; border-radius: 8px;" />
                            <input type="password" id="auth-pass" placeholder="Password" style="width: 100%; padding: 12px; margin: 10px 0; background: #111; color: #fff; border: 1px solid #444; border-radius: 8px;" />
                            <button id="auth-login-btn" style="width: 100%; padding: 12px; background: #db2777; color: white; border: none; border-radius: 8px; font-weight: bold; cursor: pointer; margin-top: 10px; transition: all 0.3s;">Pledge Soul</button>
                        </div>
                    `;
                    ...
                    document.getElementById('auth-login-btn').onclick = async () => {
                        ...
                        try {
                            ...
                            // UX: Auto-create a profile if 0 profiles, auto-select if 1
                            ...
                            if (profs.length === 0) {
                                ...
                                // Reload to show profile selector if > 1
                                window.location.reload();
                            }
                        } catch (err) {
                            alert("Auth Failed: " + err.message);
                            btn.textContent = "Pledge Soul";
                        }
                    };
                }
                resolve(false);
